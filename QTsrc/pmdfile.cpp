...
namespace mmd
{
    ...
    bool ReadHeader(PMDFile* pmdFile, File& file)
    {
        ...
        if (header.m_magic.ToQString() != "Pmd")
        {
            MMD__ERROR("PMD Header Error");
            return false;
        }

        if (header.m_version != 1.0f)
        {
            MMD__ERROR("PMD Version Error");
            return false;
        }
    ...
    bool ReadBoneDisplayList(PMDFile* pmdFile, File& file)
    {
        ...
        pmdFile->m_boneDisplayLists.resize(quint64(displayListCount) + 1);
        bool first = true;
        ...
        for (quint32 displayIdx = 0; displayIdx < displayCount; displayIdx++)
        {
            quint16 boneIdx = 0;
            Read(&boneIdx, file);
    ...
    bool ReadExt(PMDFile* pmdFile, File& file)
    {
        ...
        if (pmdFile->m_header.m_haveEnglishNameExt != 0)
        {
            ...
            quint64 numBlensShape = pmdFile->m_morphs.size();
            for (quint64 bsIdx = 1; bsIdx < numBlensShape; bsIdx++)
            {
            ...
            quint64 numBoneDisplayName = pmdFile->m_boneDisplayLists.size();
            for (quint64 displayIdx = 1; displayIdx < numBoneDisplayName; displayIdx++)
            {
    ...
    bool ReadPMDFile(PMDFile* pmdFile, File& file)
    {
        ...
        {
            MMD__ERROR("ReadHeader Fail.");
            return false;
        }

        if (!ReadVertex(pmdFile, file))
        {
            MMD__ERROR("ReadVertex Fail.");
            return false;
        }

        if (!ReadFace(pmdFile, file))
        {
            MMD__ERROR("ReadFace Fail.");
            return false;
        }

        if (!ReadMaterial(pmdFile, file))
        {
            MMD__ERROR("ReadMaterial Fail.");
            return false;
        }

        if (!ReadBone(pmdFile, file))
        {
            MMD__ERROR("ReadBone Fail.");
            return false;
        }

        if (!ReadIK(pmdFile, file))
        {
            MMD__ERROR("ReadIK Fail.");
            return false;
        }

        if (!ReadBlendShape(pmdFile, file))
        {
            MMD__ERROR("ReadBlendShape Fail.");
            return false;
        }

        if (!ReadBlendShapeDisplayList(pmdFile, file))
        {
            MMD__ERROR("ReadBlendShapeDisplayList Fail.");
            return false;
        }

        if (!ReadBoneDisplayList(pmdFile, file))
        {
            MMD__ERROR("ReadBoneDisplayList Fail.");
            return false;
        }

        quint64 toonTexIdx = 1;
        for (auto& toonTexName : pmdFile->m_toonTextureNames)
        {
            std::stringstream ss;
        ...
        if (file.Tell() < file.GetSize())
        {
            ...
            {
                MMD__ERROR("ReadToonTextureName Fail.");
                return false;
            }
        }
        ...
        if (file.Tell() < file.GetSize())
        {
            ...
            {
                MMD__ERROR("ReadRigidBodyExt Fail.");
                return false;
            }
        }
    ...
    {
        MMD__INFO("PMD File Open. {}", filename);

        File file;
        if (!file.Open(filename))
        {
            MMD__INFO("PMD File Open Fail. {}", filename);
            return false;
        }

        if (!ReadPMDFile(pmdFile, file))
        {
            MMD__INFO("PMD File Read Fail. {}", filename);
            return false;
        }
        MMD__INFO("PMD File Read Successed. {}", filename);
