...
namespace mmd
{
    ...
    public:
        Logger()
        {
            ...
            m_logger = std::make_shared<spdlog::logger>("mmd", defaultSink);
        }
...
#define MMD__INFO(message, ...)\
    mmd::Info(message, ##__VA_ARGS__)

#define MMD__WARN(message, ...)\
    mmd::Warn(message, ##__VA_ARGS__)

#define MMD__ERROR(message, ...)\
    mmd::Error(message, ##__VA_ARGS__)

#define MMD__ASSERT(expr)\
    assert(expr)
